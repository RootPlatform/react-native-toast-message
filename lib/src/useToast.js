...
export function useToast({ defaultOptions }) {
    ...
    const hide = React.useCallback((params) => {
    ...
    const show = React.useCallback((params) => {
        ...
        const { text1 = DEFAULT_DATA.text1, text2 = DEFAULT_DATA.text2, type = initialOptions.type, text1Style = initialOptions.text1Style, text2Style = initialOptions.text2Style, position = initialOptions.position, autoHide = initialOptions.autoHide, visibilityTime = initialOptions.visibilityTime, topOffset = initialOptions.topOffset, bottomOffset = initialOptions.bottomOffset, keyboardOffset = initialOptions.keyboardOffset, onShow = initialOptions.onShow, onHide = initialOptions.onHide, onHidden = initialOptions.onHidden, onPress = initialOptions.onPress, swipeable = initialOptions.swipeable, translateYFactor = initialOptions.translateYFactor, animationProps = initialOptions.animationProps, props = initialOptions.props } = params;
        setData({
            text1,
            text2
        });
