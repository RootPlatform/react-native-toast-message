...
export function AnimatedContainer({ children, isVisible, position, topOffset, bottomOffset, keyboardOffset, onHide, onHidden, onRestorePosition = noop, swipeable, translateYFactor, animationProps }) {
    const { log } = useLogger();
    ...
    React.useLayoutEffect(() => {
        if (isVisible === undefined)
            return; // Avoid animating toValue 0 when launching app
